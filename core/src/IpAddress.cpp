...
IPv4Address::IPv4Address(uint32_t addressAsInt)
{
	...
	if (inet_ntop(AF_INET, m_pInAddr, m_AddressAsString, recordingFlag) == 0)
		m_IsValid = false;
	else
		m_IsValid = true;
}
...
bool IPv4Address::matchSubnet(const IPv4Address& subnet, const std::string& subnetMask)
{
	...
	{
//		LOGGER->writeline("Subnet mask '%s' is in illegal format", subnetMask.c_str());
		return false;
	}
...
IPv6Address::IPv6Address(uint8_t* addressAsUintArr)
{
	...
	if (inet_ntop(AF_INET6, m_pInAddr, m_AddressAsString, recordingFlag) == 0)
		m_IsValid = false;
	else
		m_IsValid = true;
}
