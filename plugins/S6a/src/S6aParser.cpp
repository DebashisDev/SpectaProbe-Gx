...
uint16_t S6aParser::decodeHeader(const BYTE packet, MPacket *msgObj, uint16_t chunkNo)
{
	...
	{
		printf("*** Version :: %d | Reject \n", VAL_BYTE(packet));
		return 0;
	}

	headerOffset++;

	gx_Length = getLength(packet, headerOffset);
	...
	msgObj->Diameter[chunkNo].reqRspFlag = packet[headerOffset] >> 7;
	headerOffset += 1;

	msgObj->Diameter[chunkNo].command = getLength(packet, headerOffset);
	headerOffset += 3;

	printf("*** Command:: %s|Code:: %d| Len:: %d\n", diameterStaticMap::commandCodeType[msgObj->Diameter[chunkNo].command].c_str(), msgObj->Diameter[chunkNo].command, gx_Length);

	msgObj->Diameter[chunkNo].appId = VAL_ULONG(packet + headerOffset);
	...
	msgObj->Diameter[chunkNo].hopByHopId = VAL_ULONG(packet + headerOffset);

	headerOffset += 8;				/* End-to-End Identifier */
...
VOID S6aParser::decodeAVP(const BYTE packet, MPacket *msgObj, uint16_t &chunkNo)
{
	...
	while((msgObj->Diameter[chunkNo].length - DIAMETER_HEADER) > index)
	{
		uint16_t avpLen = decodeElement(packet, msgObj, chunkNo, index);
...
VOID S6aParser::fn_getSubscriptionData(const BYTE packet, MPacket *msgObj, uint16_t chunkNo, uint16_t &offset, uint16_t &avpLen)
{
	...
	while((avpLen - 12) > subAvpLen) {
		uint16_t len = decodeElementSubsData(buffer, msgObj, chunkNo, subAvpLen);
...
uint16_t S6aParser::decodeElementSubsData(const BYTE packet, MPacket *msgObj, uint16_t chunkNo, uint16_t &avpLen)
{
	uint16_t offset = avpLen;
...
//	printf("Subscriber Data Code: %d| Len: %d\n", avpCode, AVPLen);
...
VOID S6aParser::fn_getMsisdn(const BYTE packet, MPacket *msgObj, uint16_t chunkNo, uint16_t &offset, uint16_t &avpLen)		// TODO
{
    ...
    sprintf(Msisdn, "%02x%02x%02x%02x%02x", ((packet[offset] & 0xF0) >> 4 | (packet[offset] & 0x0F) << 4),
											...
											((packet[offset + 2] & 0xF0) >> 4 | (packet[offset + 2] & 0x0F) << 4),
    ...
    msgObj->Diameter[chunkNo].resultCode = 2001;
}
